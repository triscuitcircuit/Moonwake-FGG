...
async function getAll(req, res) {
   ...
   //
   //  if(m_ac)
   //      where.GASYMO_ARMOR_CLASS = {[Op.or]:{[Op.eq]: m_ac}}
   //
   //  if(xp_val)
   //      // TODO
...
const GASYMO_GAME_SYSTEM_MONSTER =
        await db.sequelize.models.GASYMO_GAME_SYSTEM_MONSTER.findAndCountAll({
            ...
            where: Sequelize.and(
                { ST_CODE: 'active'},
                Sequelize.or(                                                   // change this based on user global (make a different function, database page will call sep function based on user toggle? idk if back end can get fend hooks)
                    { GASYMO_DISPLAY_NAME : {[Op.like]:'%'+name +"%"}},
                    { GASYMO_ARMOR_CLASS : {[Op.or]:{[Op.eq]: m_ac}}}       // no need for tenary operator to handle nulls - seems just fine ignoring stuff user doesnt put in like this
                )
            )
        });
